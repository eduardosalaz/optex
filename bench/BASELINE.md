...
## 2026-07-18 comparison against the official Python bindings

`bench/PYTHON_COMPARISON.md` (generated by `bench/python_comparison/`)
benchmarks each Optex backend against its vendor's official Python
binding (highspy, gurobipy, docplex, coptpy) on four identical models:
objectives agree exactly on all sixteen model-engine pairs,
solver-reported times match per engine within noise, build times are the
same order of magnitude in both hosts (Optex faster on the dense QP,
2-3x slower plus handoff on the 120k-variable LP), and memory is the
same ballpark. Rerun instructions are in the report header; the venvs
are local-only.
